"""
Run the notebook's modeling pipeline on the project's REAL dataset
=================================================================
Data: product_reviews_scraped.csv — real Amazon Reviews 2023 (McAuley-Lab),
~10K reviews across 10 categories.  NO scraping.

Pipeline (same as the team notebook): balanced binary sample -> EDA
(length / word frequency / TF-IDF) -> TF-IDF + Logistic Regression ->
fine-tune DistilBERT (1 epoch, AdamW 2e-5, batch 8, max-len 128).

Writes results_real.json, used to build the Streamlit dashboard.
"""
...
# ----------------------------------------------------------------------
# 1. Load real data + balanced binary sample
# ----------------------------------------------------------------------
raw = pd.read_csv(CSV)
raw_count = len(raw)
rating_dist = {int(k): int(v) for k, v in raw["rating"].value_counts().sort_index().items()}
category_dist = {str(k): int(v) for k, v in raw["category"].value_counts().items()}
...
]).sample(frac=1, random_state=SEED).reset_index(drop=True)[["text", "label"]]
...
avg_review_length = {"Negative (0)": round(float(avg.get(0, 0)), 1),
                    "Positive (1)": round(float(avg.get(1, 0)), 1)}

import nltk
nltk.download("stopwords", quiet=True)
from nltk.corpus import stopwords
stop = set(stopwords.words("english"))
words = [w for w in " ".join(df["text"]).split() if w.lower() not in stop and len(w) > 2]
freq = Counter(words)
top_words = [[w, int(c)] for w, c in freq.most_common(20)]
top_words_full = [[w, int(c)] for w, c in freq.most_common(120)]

from sklearn.feature_extraction.text import TfidfVectorizer
tfidf20 = TfidfVectorizer(max_features=20, stop_words="english").fit(df["text"])
tfidf_features = sorted(tfidf20.get_feature_names_out().tolist())

# ----------------------------------------------------------------------
# 3. TF-IDF + Logistic Regression  (stratified 80/20)
# ----------------------------------------------------------------------
...
lr = {
    "accuracy": round(accuracy_score(yte, lrp), 4),
    "precision": round(precision_score(yte, lrp, average="weighted"), 4),
    "recall": round(recall_score(yte, lrp, average="weighted"), 4),
    "f1": round(f1_score(yte, lrp, average="weighted"), 4),
    "report": {
        "Negative (0)": {k: round(rep["0"][k], 2) for k in ["precision", "recall", "f1-score"]} | {"support": int(rep["0"]["support"])},
...
distilbert = {
    ...
    "report": {
        "Negative": {k: round(drep["0"][k], 2) for k in ["precision", "recall", "f1-score"]} | {"support": int(drep["0"]["support"])},
        "Positive": {k: round(drep["1"][k], 2) for k in ["precision", "recall", "f1-score"]} | {"support": int(drep["1"]["support"])},
    },
    "confusion": cm.tolist(),
    "correct": int(np.trace(cm)), "misclassified": int(cm.sum() - np.trace(cm)),
...
json.dump({
    ...
    "raw_count": raw_count, "rating_distribution": rating_dist, "category_dist": category_dist,
    "balanced_total": balanced_total, "label_balance": label_balance,
    "unique_reviews": unique_reviews, "duplicate_reviews": duplicate_reviews,
