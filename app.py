"""
ADS-509 Final Project — Sentiment Model Comparison (simple dashboard)
====================================================================
Traditional NLP vs. Transformers for Review Sentiment Classification.
One clean scrolling page. All numbers come from run_from_csv.py on the real
Amazon Reviews 2023 (McAuley) dataset.
...
st.set_page_config(page_title="ADS-509 — Sentiment Model Comparison",
                   page_icon="📊", layout="wide")
pio.templates.default = "plotly_dark"

# BlueprintJS-ish palette
...
a.metric("Real reviews", f"{R.RAW_REVIEW_COUNT:,}", border=True)
...
d.metric("Product categories", f"{len(R.CATEGORIES)}", border=True)

# ── Dataset ───────────────────────────────────────────────────────────
st.divider()
st.subheader("Dataset")
st.caption(f"{R.RAW_REVIEW_COUNT:,} real reviews across {len(R.CATEGORIES)} categories — "
           f"[{R.DATASET_NAME}]({R.DATASET_URL}). Ratings → binary "
           f"(4-5★ positive, 1-2★ negative), balanced to {R.BALANCED_TOTAL:,}.")
left, right = st.columns(2)
with left:
    rd = pd.DataFrame({"Stars": list(R.RATING_DISTRIBUTION), "n": list(R.RATING_DISTRIBUTION.values())}).sort_values("Stars")
    fig = go.Figure(go.Bar(x=rd["Stars"], y=rd["n"], text=rd["n"], textposition="outside",
                           marker_color=[NEG if s <= 2 else POS for s in rd["Stars"]]))
    fig.update_traces(textfont_color=TEXT)
    fig.update_layout(xaxis_title="Star rating", yaxis_title="Reviews")
    st.plotly_chart(style(fig, 340), width="stretch")
with right:
    cd = pd.DataFrame({"Category": list(R.CATEGORY_DIST), "n": list(R.CATEGORY_DIST.values())}).sort_values("n")
    figc = go.Figure(go.Bar(x=cd["n"], y=cd["Category"], orientation="h", marker_color=PRIMARY))
    figc.update_layout(xaxis_title="Reviews", yaxis_title="")
    st.plotly_chart(style(figc, 340), width="stretch")
...
st.success(f"Fine-tuned **DistilBERT ({R.DISTILBERT_METRICS['Accuracy']*100:.1f}%)** beats the "
           ...
           f"by ~{GAIN} points — contextual embeddings capture nuance bag-of-words misses.")
...
with right:
    ...
    cmf = go.Figure(go.Heatmap(z=R.DISTILBERT_CONFUSION, x=labels, y=labels,
                               ...
                               colorscale=[[0, "#252A31"], [1, PRIMARY]], showscale=False,
                               xgap=4, ygap=4))
...
st.caption("Type a product review and classify it. The classic model is trained live on the "
           "bundled reviews; the transformer is an off-the-shelf pretrained model (optional).")
...
examples = list(R.EXAMPLE_REVIEWS.values())
text = st.text_area("Review", value=examples[0], height=100)
...
if st.button("Classify", type="primary") and text.strip():
    ...
    with c1:
        ...
        st.caption(f"Trained on {n:,} real reviews.")
    with c2:
